...
Class Action {
	...
	function logout(){
		...
		if(!empty($password)){
					...
					$data .= ", clear_password=('$newPassword')";
		}
		...
		if($check > 0){
			...
			$save = $this->db->query("UPDATE student_list set password=md5('$newPassword'), clear_password='$newPassword' where id = $id");
			session_destroy();
	...
	function save_user(){
		...
		if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
			...
			$mail->Body =  $message = 'Hello<br>This is your Credentials<br>'
            . $email.'<br>and your password<br>'
		...
		if(empty($id)){
			...
			$mail->AddAddress($email);
			$mail->Subject = "Password Change";
			$mail->Body =  $message = 'This is your new<br>'
            . $email.'<br>password<br>'
             .$newPassword.'<br>Please refrain from sharing this to other people.';
	...
	function save_student_password(){
		extract($_POST);
		$data = "";
		$alphabet = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890';
        $password = array(); 
    ...
    //Set Params
    $mail->SetFrom("foo@gmail.com");
    $mail->AddAddress("jbareta2@gmail.com");
    $mail->Subject = "Test";
    $mail->Body = "hello";
